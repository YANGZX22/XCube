...
- 🛠️ **内置工具** —— Canvas 文档、计划模式、Python 沙箱、图表、日历、Map Kit 地图与常用地点，以及 PDF／图片／DOCX／XLSX 转文本
...
| **计划模式** (`plan_mode`) | 模型写下多步任务的计划，并在每完成一步时立刻勾选，进度显示在对话旁的计划面板里 |
...
多步任务不再是黑箱。模型接到需要好几步才能完成的任务时，会调用 `plan_mode` 先写下步骤，然后**在每完成一步的当下**立即勾掉它，而不是等任务结束再一次性补勾。对话旁的计划面板会列出全部步骤和完成进度，哪些做完了、哪一步正在做、还剩什么，一眼可见。

- **勾选由模型自己负责。** 工具提示明确要求：勾选是模型的职责，且必须在完成的当下进行；只在回复正文里写“已完成”而不调用工具不算数——用户看到的是面板。
- **你也可以勾。** 点任意一步即可切换状态，模型漏勾或你自己完成了某步时用得上。
- **`/plan` 随时打开面板。** 步骤仍由模型写入，这个命令只负责让面板可见，并表明你希望它按计划推进。
- **每个会话一份计划**，随对话保存在本地，下次回到这个会话时自动恢复。

只有当某步的产出真实存在且可验证时才应该勾选，因此勾满的面板代表真实进展而不是乐观估计。一两步就能答完的简单问题不会生成计划。
