...
> [!IMPORTANT]
> **沉浸光感兼容性问题 (截至 2026 年 8 月 29 日)：** HarmonyOS `7.0.0.105` (API 26) 下，非 HDS 组件的沉浸光感目前全部失效，但 DevEco Studio 仍能正常完成编译；HDS 组件不受影响。初步判断是项目使用的 API 26 Beta2 SDK (`26.0.0.32`) 与正式系统运行时 (`26.0.0.105`) 存在兼容性差异。若该问题长期未得到修复，项目可能会考虑降级沉浸光感效果。
