...
- **Email import** — an Email category alongside PDF, Word, spreadsheets, and images can import selected IMAP messages in one tap; message bodies, supported attachments, and inline images enter the same parsing/OCR pipeline. Mailbox authorization is configured under **Settings → Tools → Email**
- **Outlook OAuth2** — Outlook.com and Microsoft 365 use device-code authorization with IMAP/SMTP XOAUTH2. Supply a Microsoft Entra public-client ID with device-code flow and the `IMAP.AccessAsUser.All`, `SMTP.Send`, and `offline_access` delegated scopes. The refresh token is protected by the system secure asset store
- **NetEase IMAP ID** — 163, 126, and yeah.net sessions send RFC 2971 client identification after authentication, preventing NetEase's `Unsafe Login` restriction while exposing no mailbox credential
...
| **Python sandbox**             | Runs Python for calculations, data processing, and intermediate reasoning                                                                                                                                                                                                                                                       |
...
| **Calendar**                   | Reads a user-confirmed date range or creates calendar events under explicit privacy controls                                                                                                                                                                                                                                    |
| **Email**                      | Reads an authorized inbox through IMAP or sends mail through SMTP. Every model-initiated read and send requires explicit approval; app passwords are kept in the system secure asset store and are never exposed to the model                                                                                                         |
